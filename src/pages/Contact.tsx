...
export default function Contact() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-20 pt-32">
      ...
      <h1 className="font-serif text-5xl font-bold text-gold mb-12 text-center">{isArabic ? 'اتصل بنا' : 'Contact Us'}</h1>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
        <div className="flex flex-col gap-8">
          <h2 className="text-2xl font-bold text-white mb-4">{isArabic ? 'تواصل معنا' : 'Connect with the Legacy'}</h2>
          <p className="text-gray-300 leading-relaxed italic">
            {isArabic ? 'من ثلاثين سنة، كنا المسرح لأعز احتفالات الدمام. سواء تخطط لإنجاز مؤسسي أو لحظة خاصة، بيتنا على الكورنيش بيتك.' : "For three decades, we have been the stage for Dammam's most cherished celebrations. Whether you are planning a corporate milestone or a private moment, our home on the Corniche is yours."}
          </p>
          ...
          <div className="flex flex-col gap-6 mt-4">
            <div className="flex items-start gap-4 text-gray-300">
              ...
              <div>
                <h3 className="font-bold text-white mb-1">{isArabic ? 'جوال' : 'Phone'}</h3>
                <p>057 096 2002</p>
              </div>
            </div>
            <div className="flex items-start gap-4 text-gray-300">
              ...
              <div>
                <h3 className="font-bold text-white mb-1">{isArabic ? 'بريد' : 'Email'}</h3>
            ...
            <div className="flex items-start gap-4 text-gray-300">
              ...
              <div>
                <h3 className="font-bold text-white mb-1">{isArabic ? 'العنوان' : 'Our Stage'}</h3>
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-8">
          <h3 className="font-bold text-white uppercase tracking-wide mb-6">{isArabic ? 'أرسل رسالة' : 'Send a Message'}</h3>
          <form className="flex flex-col gap-4">
            ...
            />
            <textarea 
              placeholder={isArabic ? 'الرسالة' : 'Your Message'} 
              rows={4}
              className="w-full rounded-xl border border-white/10 bg-white/5 p-4 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-gold focus:ring-1 focus:ring-gold"
            ></textarea>
            <button 
